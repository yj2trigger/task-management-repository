...
| [ADR-007](decisions/ADR-007-iot-polling-strategy.md) | Adaptive Polling | Tuya quota 67% 유지, Mode별 interval + 심야 감소 (734,400 → 17,460 호출/월) |
...
**API Quota 설계:**  
단순 폴링(734,400 호출/월)은 무료 플랜(26,000/월)의 28×. Batch API + Mode별 adaptive interval + 심야 감소로 17,460 호출/월(97.6% 절감)으로 quota 67% 유지.  
가용 대수가 많을수록(Mode A) 빠른 polling — 헛걸음 방지 우선. Mode C는 대기열 알림이 primary path이므로 느린 polling 허용.
