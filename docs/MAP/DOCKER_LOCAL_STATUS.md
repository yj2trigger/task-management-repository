...
**시행착오**: "VS Code 옛 확장 버전 정리"에서 ChatGPT(Codex) 확장 폴더를 지울 때, VS Code 등록 목록엔 새 버전만 있는 걸 확인하고 옛 폴더를 삭제했으나 **그 창의 프로세스(PID 7108)가 여전히 옛 버전을 붙잡고 있는지는 확인하지 않았음**. 결과: 옛 폴더에 `codex.exe`만 남고 나머지 파일이 지워져 그 창의 ChatGPT 확장이 오동작할 뻔함. `Developer: Reload Window`로 새 버전 전환 후 남은 폴더 정리해 복구. **교훈**: 확장 폴더 삭제 전엔 "등록 목록"뿐 아니라 "지금 그 파일을 물고 있는 프로세스"까지 확인해야 함.
