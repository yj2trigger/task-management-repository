...
## 38. depth 읽기 패딩 오류 — asShortBuffer() linear index 문제

**상황:** 로그에서 `유효: 1000~2000/14400 = 7~14%`, 나머지 86~93%가 65m(=65535mm)로 나옴. 실내 스캔에서 86%가 5m 초과는 불가능 → depth 값 자체가 오염됨.

**원인:**
ARCore depth plane에는 row padding이 존재할 수 있음.
```
Row 0: [depth×160픽셀(320bytes) | padding×N bytes]
Row 1: [depth×160픽셀(320bytes) | padding×N bytes]
```

`asShortBuffer().get(i)` — short index, linear. padding 무시:
```
get(0)~get(159):     Row 0 depth ✅
get(160)~get(160+N): Row 0 padding → 65535 쓰레기 ❌
get(161+N)~...:      Row 1 depth가 잘못된 offset으로 읽힘
```

padding bytes = 65535(=65.535m)로 채워짐 → 86%가 65m로 보임.

**수정:**
```kotlin
// asShortBuffer() + linear index → 패딩 무시 오류
// ByteBuffer + rowStride/pixelStride → 올바른 픽셀 접근
val dp = depthImg.planes[0]
val byteBuffer = dp.buffer.order(ByteOrder.LITTLE_ENDIAN)
val rowStride = dp.rowStride
val pixelStride = dp.pixelStride  // depth16 = 2 bytes
for (row in 0 until dh) for (col in 0 until dw) {
    val mm = byteBuffer.getShort(row * rowStride + col * pixelStride).toInt() and 0xFFFF
