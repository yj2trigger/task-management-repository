# MAP — 2026년도 인공지능 루키 본선 제안서 초안 (v4)
...
**범례** — ✅ 확인됨(코드/레포/도전제안서 근거) · 📝 초안(근거는 있으나 팀 검토·톤 조정 필요) · ⚠️ 정보 부족(팀이 직접 채워야 함, 비워둠) · 🚧 미완성(1차 PoC 완료, 2차 기간 완성 계획)
...
### 🚧 v4 정정: 모빌리티 반경 필터, 1차 기간엔 미완성 상태 (팀 확인·코드 재검증 결과)

원문의 핵심 차별점("모빌리티 제약 반영 경로 추천")을 실제로 실현하는 `rules_filter` 노드를 다시 읽었다. 반경 상수·거리계산 로직(hub)은 정상 구현돼 있지만, 그 반경을 잴 **기준점(사용자 출발지)이 아직 요청 스키마에 없다**:

1. **사용자 출발지 좌표가 API 어디에도 없다.** `RecommendRequest`(user-service) → `AgentRequest`(agent) 전체 필드: `date/budget/theme/mobility/province/city/scheduleId/stage/exclude/places`. 위경도 출발지 필드 자체가 없고, `province/city`(행정구역 텍스트)만 있다.
2. **`rules_filter`는 이 빈자리를 "후보들 자기 자신의 좌표 평균"으로 임시 대체하고 있다.** `map-service-agent/app/nodes/agent_nodes.py::rules_filter` 주석: "origin: 좌표가 있는 후보들의 위/경도 평균(반경 필터의 중심)." 임시 방편이라 실제 필터링 효과는 약하다.
3. hub 쪽 반경 상수·`filter_by_radius` 순수함수 자체는 완성돼 있고, `test_rules_filter_keeps_subset` 테스트도 이 임시 동작을 정확히 검증하며 통과한다 — 즉 **절반은 완성, 절반(사용자 위치 입력)은 2차 기간 과제**로 남은 1차 PoC 단계 기능이다.

**이 문서의 다음 항목을 이 상태에 맞춰 "구현완료"→"1차 PoC·2차 계획"으로 정정했다: 1.5(비교표), 1.6(변경사항표), 2.2(AI 활용방안), 3.1.4/3.1.5(구현성과·한계), 3.2(기능표), 3.3.2(창의성).** 본선 제안서에는 "완성"이 아니라 "1차 PoC 완료, 2차 기간 최우선 완성 과제"로 적는 편이 팀역량·성장성 항목에도 유리함.
...
|---|---|---|---|
| 모빌리티 제약 반영 경로 추천 | 미지원(반경·지형 제약 없음) | 미지원 | 🚧 1차 PoC — 반경 상수(도보3km/자전거·킥보드10km)는 구현, 기준점을 사용자 출발지로 바꾸는 것은 2차 과제 |
...
- **여행 일정 추천**: 위 그래프. hub 규칙엔진 반경표 — 도보 3,000m / 자전거·킥보드 10,000m / 자동차·대중교통 무제한(`app/rules/rule_engine.py`의 `RADIUS_M`). 🚧 1차 기간엔 `rules_filter`가 이 반경의 기준점("origin")으로 후보 장소들 자기 좌표 평균을 임시로 쓰고 있다 — 사용자 출발지 좌표 입력은 요청 스키마에 아직 없어(1.6 참고) 2차 기간에 추가할 계획이다.
...
- **AI 추천 파이프라인(예선 핵심 아이디어의 하이브리드 구조)**: 강수확률 기반 실내가점+체류시간 추정은 규칙엔진으로, 최종 선정·동선사유는 Gemini로 분리해 Human-in-the-Loop 유지. 🚧 이동수단별 반경 필터(도보3km/자전거·킥보드10km)는 반경 로직까지는 구현했고, 기준점을 사용자 출발지로 연결하는 마지막 단계가 1차 기간엔 미완성으로 남음(위 v4 정정 참고) — 2차 기간 완성 예정.
...
- **모빌리티 반경 필터 완성** (원문 핵심 차별점, 최우선 과제) — 1차 기간엔 `RecommendRequest`/`AgentRequest`에 사용자 출발지 좌표 필드가 없어 `rules_filter`의 origin을 후보 평균으로 임시 대체해뒀음. 2차 기간에 출발지 좌표 입력을 추가하고 origin을 그 값으로 바꾸면 원문이 약속한 "모빌리티 제약 반영 추천"이 완성됨.
...
| 기능 | 개요 | 개발 진행 현황 | 진척도 |
|---|---|---|---|
| ① 여행경로 추천 | 모빌리티 제약+날씨+테마 기반 AI+규칙엔진 하이브리드 추천 | 🚧 1차 PoC — 날씨연동·다일차·캐시재사용은 완료, 모빌리티 반경 필터는 기준점(사용자 출발지) 연결이 2차 과제(1.6·2.2 참고) | ⚠️ 팀 판단 |
...
### 3.3.2 창의성·도전성 📝 초안 — 원문 주장 일부만 코드로 검증됨
원문이 내세운 "2단계 AI 에이전트 분리"와 "Human-in-the-Loop"은 실제로 지켜졌고, 장소 탐색·필터링·점수화를 LLM이 아니라 **결정론적 규칙엔진**에 맡기고 Gemini는 최종 선정·설명에만 관여하는 더 보수적인 구조로 구현됐다. LLM이 실측 데이터 없이 답을 지어낸 경우(`grounded=false`)를 명시적으로 표시해 신뢰도를 구분하는 것도 원문의 "AI 결과를 맹신하지 않는" 철학의 연장. 같은 하이브리드 철학(규칙엔진+LLM 역할분리)을 카메라 챗봇(YOLO+Gemini Vision)에도 일관되게 적용한 점, 프롬프트 인젝션 방어를 처음부터 설계에 포함시킨 점은 원문에는 없던 추가 방어.

🚧 원문이 가장 앞세운 차별점("모빌리티 제약 반영 경로추천", "국내 최초 시도")은 규칙엔진·반경 로직까지는 만들어져 있고, 마지막 한 조각(사용자 출발지 좌표를 반경 기준점으로 연결)이 1차 기간엔 미완성으로 남아 있다(위 v4 정정 참고). 본선 제안서에는 "완성"이 아니라 "1차 PoC 완료, 2차 기간 최우선 완성 과제"로 정직하게 적는 편이 팀역량·성장성 항목에도 더 유리함 — 2차 기간에 실제로 완성하면 다음 제출 때 "확인됨"으로 올릴 수 있음.
