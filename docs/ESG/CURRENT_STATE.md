...
| 방식 | 트리거 주체 | 신뢰도 | 비고 |
|---|---|---|---|
| GitHub `schedule` | GitHub 내부 스케줄러 | 낮음 | 수시간 지연·누락 확인됨 |
| cron-job.org + `workflow_dispatch` | 외부 크론 서비스 | 높음 | 분 단위 정확, 무료 |

**SSOT 원칙:** threshold는 `fly.toml [env]` 원본. config.py는 로컬 dev 기본값.
...
| 이슈 | 상태 |
|------|------|
| soft_reserved + 전력 낮음 → available 강제 전환 버그 | ✅ |
...
| 관리자 그래프 자정 전환 데이터 공백 | ✅ 롤링 24h 윈도우 |
| IoT power_w 미저장 | ✅ 수신 시 로그 저장 추가 |
| broken 기기 IoT 자동 전환 | ✅ 보호 추가 |
| relay 워크플로우 timeout 6분 초과 | ✅ 8분으로 연장 + pip 캐시 |
...
| in_use 자동 해제 | 없음 — IoT 전력 낮음 시에만 available 전환 |
